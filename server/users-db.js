...
import {
  hashPassword, verifyPasswordHash, generateResetToken, hashResetToken,
  generateOtp, hashOtp,
} from './auth.js';
...
export async function createUser({ name, email, phone, password, role = 'customer' }) {
  if (!password || password.length < 6) {
    throw new Error('Password must be at least 6 characters');
  }
  if ((email && await findByEmailOrPhone(email)) || (phone && await findByEmailOrPhone(phone))) {
...
export function publicUser(u) {
  if (!u) return null;
  return { id: u.id, name: u.name, email: u.email, phone: u.phone, role: u.role, status: u.status, mfaEnabled: u.mfa_enabled };
}
...
export async function startMfaSetup(userId, accountLabel) {
  ...
  const otpauthUrl = authenticator.keyuri(accountLabel, 'Priyasa Fashion', secret);
...
export async function startRegistration({ name, email, phone, password }) {
  ...
  if (await findByEmailOrPhone(email)) throw new Error('An account with this email already exists');
  if (phone && await findByEmailOrPhone(phone)) throw new Error('An account with this phone already exists');
  ...
  sendOtpEmail(email, otp).catch((err) => console.error('Failed to send OTP email:', err.message));
}
...
export async function resendRegistrationOtp(email) {
  ...
  if (!pending) throw new Error('No pending registration found for this email — start again');
  ...
  sendOtpEmail(email, otp).catch((err) => console.error('Failed to resend OTP email:', err.message));
}
...
export async function verifyRegistrationOtp(email, code) {
  ...
  if (hashOtp(String(code)) !== pending.otp_hash) {
    await query(`UPDATE pending_registrations SET attempts = attempts + 1 WHERE id = $1`, [pending.id]);
...
export async function getAllCustomers() {
  ...
  return rows.map((u) => ({
    ...
    status: u.status, mfaEnabled: u.mfa_enabled, createdAt: u.created_at,
  }));
}
