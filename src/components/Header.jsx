...
export default function Header({ onOpenCart, categories, onSelectCategory }) {
  ...
  function handleAddressesClick() {
    closeAccountMenu();
    notify("Address book is coming soon — for now, you'll enter your address at checkout each time.");
  }
  ...
  return (
    <>
      ...
      >
        <Toolbar sx={{ maxWidth: 1200, mx: 'auto', width: '100%', height: { xs: 64, sm: 78 }, px: { xs: 1.5, sm: 4 } }}>
          <Box
            component="button"
            onClick={() => navigate('/')}
            aria-label="Priyasa Fashion — go to homepage"
            sx={{
              mr: { xs: 'auto', md: 4 }, display: 'flex', minWidth: 0,
              background: 'none', border: 0, p: 0, cursor: 'pointer',
            }}
          >
            <Logo size={30} wordmarkSize={19} wordmarkSx={{ display: { xs: 'none', sm: 'block' } }} />
          </Box>
          ...
          <Stack direction="row" alignItems="center" spacing={1}>
            <Tooltip title={isLoggedIn ? customer?.name || 'Your account' : 'Sign in'}>
              <IconButton onClick={handleAccountClick} aria-label="Your account" sx={{ color: 'primary.contrastText' }}>
            ...
            >
              <Box sx={{ px: 2, py: 1.25 }}>
                <Typography sx={{ fontWeight: 700, fontSize: 14.5 }} noWrap>{customer?.name}</Typography>
              ...
              <MenuItem onClick={handleAddressesClick}>
                <ListItemIcon><PlaceOutlinedIcon fontSize="small" /></ListItemIcon>
