...
import { notifyLogin, notifyLogout } from "./cartBridge.js";

const CustomerAuthContext = createContext(null);
const TOKEN_KEY = "priyasafashion_customer_token";
const USER_KEY = "priyasafashion_customer_info";

// Snapshot the guest cart from localStorage before login overwrites state
function loadCart() {
  try {
    return JSON.parse(
      window.localStorage.getItem("priyasafashion_cart") || "[]",
    );
  } catch {
    return [];
  }
...
export function CustomerAuthProvider({ children }) {
  ...
  const resendRegistrationOtp = useCallback(async () => {
    ...
    try {
      await api("/api/auth/register/resend", {
        method: "POST",
    ...
    async (identifier, password) => {
      ...
      try {
        ...
        if (data.user.role !== "customer") {
          setError(
            "This is an admin account — sign in from the Admin Dashboard instead.",
          );
          return false;
        }
        persist(data.token, data.user);
  ...
  const confirm2fa = useCallback(
    async (code) => {
      return api("/api/auth/2fa/confirm", {
        method: "POST",
        token,
        body: { code },
      });
