...
import { registerCartCallbacks } from "./cartBridge.js";

const StoreContext = createContext(null);

export function StoreProvider({ children }) {
  const [products, setProducts] = useState(() =>
    loadJSON("products", seedProducts),
  );
  ...
  const refreshProducts = useCallback(async () => {
    ...
    try {
      ...
      console.warn(
        "Could not load products from backend, using local copy:",
        err,
      );
  ...
  useEffect(() => {
    if (productsSource === "local") saveJSON("products", products);
  ...
  const token = () =>
    window.localStorage.getItem("priyasafashion_customer_token");

  // Push current cart to server (fire-and-forget)
  const pushCart = useCallback((items) => {
    const t = token();
    if (!t || !paymentConfig.backendBaseUrl) return;
    fetch(`${paymentConfig.backendBaseUrl}/api/cart`, {
      method: "PUT",
      headers: {
  ...
  // On login: merge guest cart with server cart, then load server cart
  const syncCartOnLogin = useCallback(async (localCart) => {
    const t = token();
    if (!t || !paymentConfig.backendBaseUrl) return;
    try {
      const res = await fetch(`${paymentConfig.backendBaseUrl}/api/cart/sync`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${t}`,
      ...
      console.warn("Cart merge on login failed:", err);
    }
  }, []);
  ...
  // Register callbacks so CustomerAuthContext can trigger these
  useEffect(() => {
    registerCartCallbacks({
      syncOnLogin: syncCartOnLogin,
      clearOnLogout: clearCartOnLogout,
    });
  }, [syncCartOnLogin, clearCartOnLogout]);
  ...
  const placeOrder = useCallback(
    (customer, paymentMethod, paymentRef) => {
      const order = {
        id: generateOrderId(),
      ...
      if (paymentConfig.backendBaseUrl) {
        const customerToken = window.localStorage.getItem(
          "priyasafashion_customer_token",
        );
        fetch(`${paymentConfig.backendBaseUrl}/api/orders`, {
          ...
          console.warn(
            "Order sync to backend failed (order still saved locally):",
            err,
          ),
        );
      }
